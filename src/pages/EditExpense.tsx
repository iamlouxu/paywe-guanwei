...
const EditExpense: React.FC = () => {
        ...
        const fetchMembersAndUser = async () => {
            ...
            if (expenseId) {
                ...
                if (splits && splits.length > 0) {
                    setSplitUsers(splits.map(s => s.user_id));
                } else if (user) {
                    // 萬一沒抓到就預設自己
                    setPaidBy(user.id);
                }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (expenseError) {
                throw new Error(expenseError.message || '更新花費失敗');
            }
    ...
    const selectedPaidByMember = members.find(m => m.id === paidBy);
        ...
        <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 min-h-screen">
            <div className="relative flex min-h-screen w-full flex-col max-w-md mx-auto shadow-2xl">
                ...
                <div className="flex items-center p-4 sticky top-0 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md z-10 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <h2 className="text-lg font-bold flex-1 text-center">編輯花費</h2>
                    ...
                    <div className="mb-6 gap-2 flex flex-col relative" ref={dropdownRef}>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <UserAvatar 
                                    src={selectedPaidByMember?.avatar_url} 
                                    username={selectedPaidByMember?.username || '選擇付款人'} 
                                    size="sm" 
                                />
                                <span>{selectedPaidByMember?.username || '選擇付款人'}</span>
                            </div>
                            <motion.span 
